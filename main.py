...
from nlptoolkit.clustering.models.DGI.query_embedding import class_query_embedding
...
if __name__ == "__main__":
    ...
    parser.add_argument("--query", type=str, default="cancer is bad", help="query")
    ...
    if args.infer == 1:
        ...
        # embeddings = inferer.infer_embeddings()
        # print(len(embeddings))  
        query = "Covid cancer risk"
        df = pd.DataFrame(data={'text': [query]})
        df.to_csv("./data/query.csv", columns=df.columns)

        query_embeddings_obj = class_query_embedding(args)
        #query_embeddings_obj.get_query_embedding()
        # pca, pca_embeddings = inferer.PCA_analyze(n_components=2)
        # tsne_embeddings = inferer.plot_TSNE(plot=True)
        # num_embeddings = len(tsne_embeddings)
        # num_papers_to_retrieve = 5
        # result = inferer.cluster_tsne_embeddings(tsne_embeddings,\
        #                                          n_start=num_embeddings//num_papers_to_retrieve, n_stop=num_embeddings//num_papers_to_retrieve, method='ac', plot=True)
